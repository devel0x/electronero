  ...
  try {
    ...
    adminNodesEl.innerHTML = entries
      .map((entry) => {
        ...
        const uptime = node.uptime || { uptime_ratio: 0, successful_checks: 0, total_checks: 0, average_latency_ms: null };
        const statusBadge =
        ...
        <article class="node-card">
          ...
          <div class="node-meta">
            <span>P2P: ${node.p2p_host}:${node.p2p_port}</span>
            <span>RPC: ${node.rpc_host}:${node.rpc_port}</span>
            <span>Latency: ${uptime.average_latency_ms ? uptime.average_latency_ms.toFixed(1) : "–"} ms</span>
          </div>
          <footer class="admin-node-actions">
            <span>Uptime: ${(uptime.uptime_ratio * 100).toFixed(2)}%</span>
