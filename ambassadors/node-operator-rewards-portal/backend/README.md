...
PORTAL_ADMIN_PORTAL_PASSWORD="super-secure-passphrase"
```
...
When `PORTAL_ADMIN_PORTAL_PASSWORD` is configured, every request hitting an `/admin/*` endpoint must include an
`X-Admin-Portal-Key` header containing that password. Admin tokens expose the following additional routes:
