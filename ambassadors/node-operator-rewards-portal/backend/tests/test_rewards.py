...
from app.models import AuthenticatedUser, NodeRead, NodeStatus, UptimeScore
...
async def test_admin_portal_password_required(monkeypatch):
    monkeypatch.setenv("PORTAL_SECRET_KEY", "x" * 32)
    monkeypatch.setenv("PORTAL_ADMIN_PORTAL_PASSWORD", "s3cretKey")
    get_settings.cache_clear()

    from app.dependencies import require_admin_access

    admin_user = AuthenticatedUser(id="42", email="admin@example.com", is_admin=True)

    with pytest.raises(Exception) as exc:
        await require_admin_access(user=admin_user, portal_key=None)
    assert getattr(exc.value, "status_code", None) == 403

    with pytest.raises(Exception) as exc:
        await require_admin_access(user=admin_user, portal_key="not-it")
    assert getattr(exc.value, "status_code", None) == 403

    result = await require_admin_access(user=admin_user, portal_key="s3cretKey")
