...
from .dependencies import get_current_user, require_admin_access
from .models import (
    AdminNodeDetail,
    AuthenticatedUser,
    DashboardSummary,
...
async def get_pool_state(_: AuthenticatedUser = Depends(require_admin_access)) -> PoolState:
...
async def adjust_pool_balance(
    ...
    _: AuthenticatedUser = Depends(require_admin_access),
) -> PoolState:
...
async def get_admin_nodes(_: AuthenticatedUser = Depends(require_admin_access)) -> List[AdminNodeDetail]:
...
async def reject_node(node_id: str, _: AuthenticatedUser = Depends(require_admin_access)) -> AdminNodeDetail:
    node = await repo.get_node(node_id)
    if not node:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node not found")
...
async def reinstate_node(node_id: str, _: AuthenticatedUser = Depends(require_admin_access)) -> AdminNodeDetail:
    node = await repo.get_node(node_id)
    if not node:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Node not found")
...
async def export_payouts(_: AuthenticatedUser = Depends(require_admin_access)) -> List[PayoutExportRow]:
