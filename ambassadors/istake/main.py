...
async def _require_user(request: Request) -> str:
    ...
    email = await redis_client.get(f"session:{token}")
    if not email:
        raise HTTPException(status_code=401, detail="invalid session")
    return _norm_key(email)


async def _status_for_user(user: str) -> Dict[str, Any]:
...
@app.post("/stake")
async def stake(req: StakeRequest, request: Request) -> Dict[str, Any]:
...
@app.post("/unstake")
async def unstake(req: UnstakeRequest, request: Request) -> Dict[str, Any]:
    user = await _require_user(request)
...
async def staking_status(request: Request) -> Dict[str, Any]:
    user = await _require_user(request)
    return await _status_for_user(user)
...
async def user_dashboard(request: Request) -> HTMLResponse:
    user = await _require_user(request)
    data = await _status_for_user(user)
    return templates.TemplateResponse(
        "user.html", {"request": request, "user_id": user, **data}
    )


@app.get("/user/{email}", response_class=HTMLResponse)
async def user_dashboard_admin(request: Request, email: str) -> HTMLResponse:
    data = await _status_for_user(_norm_key(email))
    return templates.TemplateResponse(
