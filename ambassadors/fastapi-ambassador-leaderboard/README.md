...
The wallet section includes export buttons so administrators can download all ambassadors' scores, pending rewards, wallets, emails, and Telegram handles as JSON or CSV files. Downloads prompt for a password defined in the `GHOST_EXPORT_KEY` environment variable. It now also offers a PBST generator that uses `interchained-cli` to build a partially signed transaction paying each ambassador's pending reward to their registered wallet for offline signing.
...
- `POST /admin/pbst` – generate a PBST paying pending rewards to each registered wallet (admin).
