...
The wallet section includes export buttons so administrators can download all ambassadors' scores, pending rewards, wallets, emails, Telegram handles, and verification status as JSON or CSV files. Downloads prompt for a password defined in the `GHOST_EXPORT_KEY` environment variable.
...
- `GET /api/admin/export?fmt=json|csv&ghost=GHOST` – download scores, pending rewards, wallets, emails, Telegram handles, and verification status (admin only, requires `GHOST_EXPORT_KEY`).
