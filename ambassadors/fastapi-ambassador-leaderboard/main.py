...
async def _all_wallets() -> list[dict[str, Any]]:
    ...
    for key in keys:
        ...
        # referral registrations for this user, defaulting to 0
        ref_code = str(udata.get("referral", "")).strip().lower()
        try:
            ref_count = int(await redis_client.hget(f"referral:{ref_code}", "registrations") or 0) if ref_code else 0
        except Exception:
            ref_count = 0
    ...
    export_rows = [
        {
            ...
            "referrals": int(w.get("referrals", 0)),
        }
        for w in wallets
    ]
    if fmt.lower() == "csv":
