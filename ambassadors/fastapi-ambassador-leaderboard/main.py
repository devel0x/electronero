...
STAKE_MIN_DAYS = max(int(os.getenv("STAKE_MIN_DAYS", "30")), 1)
...
def _stake_data_key(email: str, stake_id: str) -> str:
    email_norm = email.strip().lower()
    return f"stake:data:{email_norm}:{stake_id}"
...
async def _load_stake_meta(email: str, stake_id: str) -> dict[str, Any]:
    data = await redis_client.hgetall(_stake_data_key(email, stake_id))
    if not data:
        return {}
    meta: dict[str, Any] = {**data}
    meta["stake_id"] = stake_id
    meta.setdefault("user", email)
...
async def _user_stakes(email: str) -> list[dict[str, Any]]:
    ...
    for stake_id in stake_ids:
        meta = await _load_stake_meta(email, stake_id)
...
async def create_stake(request: Request) -> JSONResponse:
    ...
    await redis_client.hset(_stake_data_key(email, stake_id), mapping=meta)
    await redis_client.set(
...
async def claim_stake(request: Request, stake_id: str) -> JSONResponse:
    ...
    await redis_client.hset(
        _stake_data_key(email, stake_id),
        mapping={
            "status": "claimed",
