...
OTC_ORDER_LOCK_PREFIX = "otc:orders:lock:"
OTC_ORDER_LOCK_TTL_SECONDS = 15
...
OTC_PRICING_MARKET = "market"
OTC_PRICING_LABELS = {
    OTC_PRICING_MARKET: "Market Rate",
}

OTC_FX_CACHE_KEY = "otc:fx:usd"
OTC_FX_CACHE_TTL = 600
OTC_FX_SOURCE_URL = os.getenv("OTC_FX_SOURCE_URL", "https://open.er-api.com/v6/latest/USD")
OTC_FX_SOURCE_LABEL = "USD FX feed"
OTC_USDX_CODES = {"USDX", "USDx"}
...
async def _otc_currency(code: str) -> dict[str, Any] | None:
    ...
    market_rate = _otc_parse_float(data.get("market_rate"))
    market_source = _safe_text(data.get("market_source", ""))
    return {
        "code": code_norm,
...
def _otc_parse_rates_payload(payload: dict[str, Any]) -> dict[str, float]:
    ...
    for code, value in payload.items():
        ...
        if val <= 0:
            continue
        cleaned[str(code).upper()] = val
    return cleaned


async def _otc_fx_rates(force_refresh: bool = False) -> dict[str, float]:
    cached_rates: dict[str, float] | None = None
    cached_raw: str | None = None
    if not force_refresh:
        cached_raw = await redis_client.get(OTC_FX_CACHE_KEY)
        if cached_raw:
            try:
                cached_rates = _otc_parse_rates_payload(json.loads(cached_raw))
            except Exception:
                cached_rates = None
            if cached_rates:
                return cached_rates
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(OTC_FX_SOURCE_URL)
        response.raise_for_status()
        payload = response.json()
        rates_payload = payload.get("rates", {}) if isinstance(payload, dict) else {}
        rates = _otc_parse_rates_payload(rates_payload)
        if rates:
            await redis_client.set(OTC_FX_CACHE_KEY, json.dumps(rates), ex=OTC_FX_CACHE_TTL)
            return rates
    except Exception:
        pass

    if cached_rates is None and cached_raw:
        try:
            cached_rates = _otc_parse_rates_payload(json.loads(cached_raw))
        except Exception:
            cached_rates = None
...
def _otc_market_quote_for_currency(
    ...
    now_iso = datetime.utcnow().isoformat()
    override_rate = _otc_parse_float(currency.get("market_rate"))
    override_source = _safe_text(currency.get("market_source", ""))
    if override_rate > 0:
        source = override_source or "Admin supplied market rate"
        return {
            "currency": code,
            "price": override_rate,
            "source": source,
            "mode": OTC_PRICING_MARKET,
            "timestamp": now_iso,
            "available": True,
        }

    if code == "ITC":
        price = _otc_parse_float(cached_data.get("itc_to_igp"))
        return {
            "currency": code,
            "price": price,
            "source": "IGP ↔ ITC pool ratio",
            "mode": OTC_PRICING_MARKET,
            "timestamp": now_iso,
            "available": price > 0,
        }

    if code in OTC_USDX_CODES:
        return {
            "currency": code,
            "price": 1.0,
            "source": "USDx pegged 1:1 with IGP",
            "mode": OTC_PRICING_MARKET,
            "timestamp": now_iso,
            "available": True,
        }
    ...
    if rate and rate > 0:
        try:
            igp_per_unit = 1.0 / float(rate)
        except Exception:
            igp_per_unit = 0.0
        available = igp_per_unit > 0
        return {
            "currency": code,
            "price": igp_per_unit if available else 0.0,
            "source": f"{OTC_FX_SOURCE_LABEL} (USD base)",
            "mode": OTC_PRICING_MARKET,
            "timestamp": now_iso,
            "available": available,
        }
    ...
    return {
        "currency": code,
        "price": 0.0,
        "source": "No market rate available",
        "mode": OTC_PRICING_MARKET,
        "timestamp": now_iso,
        "available": False,
    }
...
async def _otc_market_quotes(
    currencies: list[dict[str, Any]],
    cached_data: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    fx_rates = await _otc_fx_rates()
    quotes: dict[str, dict[str, Any]] = {}
    for currency in currencies:
        code = currency.get("code")
        if not code:
            continue
        quotes[code] = _otc_market_quote_for_currency(currency, cached_data, fx_rates)
    return quotes
...
def _otc_order_view(data: dict[str, Any]) -> dict[str, Any]:
    ...
    pricing_mode = (
        str(data.get("pricing_mode", OTC_PRICING_MARKET)).strip().lower()
        or OTC_PRICING_MARKET
    )
    price_source = _safe_text(data.get("price_source", ""))
    market_timestamp = _safe_text(data.get("market_timestamp", ""))
    view = {
        "id": data.get("id", ""),
        "status": status,
        "side": side,
    ...
    view["side_badge"] = "bg-primary" if side == "bid" else "bg-secondary"
    view["pricing_mode_label"] = OTC_PRICING_LABELS.get(
...
async def otc_page(
    ...
    data = await _get_cached_data()
    market_quotes = await _otc_market_quotes(currencies, data)
    ...
    selected_quote = market_quotes.get(selected_currency, {
    ...
    selected_price = _otc_parse_float(selected_quote.get("price"))
    return templates.TemplateResponse(
        "otc.html",
        {
            "request": request,
            ...
            "selected_market_quote": selected_quote,
            "selected_market_price": selected_price,
            "selected_market_price_display": _otc_format_decimal(selected_price, 6, grouping=True),
            "selected_market_available": bool(selected_quote.get("available")),
...
async def otc_submit(
    ...
    currency: str = Form(...),
    amount: str = Form(...),
    pricing_mode: str = Form(OTC_PRICING_MARKET),
) -> Any:
    email = await _current_email(request)
    if not email:
        return RedirectResponse("/login")
    ...
    pricing_mode_norm = (
        str(pricing_mode or OTC_PRICING_MARKET).strip().lower()
        or OTC_PRICING_MARKET
    )
    if pricing_mode_norm != OTC_PRICING_MARKET:
        return await otc_page(
            request,
            error="Only market rate buy orders are supported at this time.",
            currency=currency_code,
        )
    ...
    data = await _get_cached_data()
    currency_entry = next(
        (entry for entry in currencies if entry.get("code") == currency_code),
        {"code": currency_code, "label": currency_code},
    )
    fx_rates = await _otc_fx_rates()
    quote = _otc_market_quote_for_currency(currency_entry, data, fx_rates)
    if not quote.get("available"):
        refreshed_rates = await _otc_fx_rates(force_refresh=True)
        if refreshed_rates:
            quote = _otc_market_quote_for_currency(
                currency_entry, data, refreshed_rates
            )
    price_value = _otc_parse_float(quote.get("price"))
    if price_value <= 0:
        source_note = quote.get("source") or "market data"
        return await otc_page(
            request,
            error=f"Unable to derive a market rate from {source_note}. Please try again later.",
            currency=currency_code,
        )
    ...
    lock_key = f"{OTC_ORDER_LOCK_PREFIX}{email_norm}"
    lock_acquired = await redis_client.set(
        lock_key,
        "1",
        nx=True,
        ex=OTC_ORDER_LOCK_TTL_SECONDS,
    )
    if not lock_acquired:
        return await otc_page(
            request,
            error="An OTC order is already processing. Please wait a moment and try again.",
            currency=currency_code,
        )
    ...
    try:
        ...
        mapping: dict[str, Any] = {
            ...
            "market_timestamp": quote.get("timestamp", now.isoformat()),
        }
        ...
        success_message = (
            f"Placed market buy for {_otc_format_decimal(amount_value, 6)} "
