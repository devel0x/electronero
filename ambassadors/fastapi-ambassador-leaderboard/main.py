...
async def _record_transfer(
    ...
    *,
    ...
    mode = "leaderboard" if impacts_leaderboard else "guardian"
...
async def transfers_submit(
    ...
    await _record_transfer(
        ...
        impacts_leaderboard=not guardian_mode,
    )

    dest_display = destination_profile.get("telegram") or destination_profile.get("email_display") or receiver_email
    if guardian_mode:
        success_message = (
            f"Transferred {amount_value:.2f} guardian points to {dest_display} (no leaderboard impact)"
        )
    else:
