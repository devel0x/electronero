...
async def wallet_form(request: Request) -> Any:
    ...
    kyc_status = _normalize_kyc_status(user.get("kyc_status"))
    return templates.TemplateResponse(
        "wallet.html",
        {
            "request": request,
...
async def profile_meta_update(
    ...
    return RedirectResponse(f"/?profile_msg={msg}#profile", status_code=303)
...
async def profile_kyc_update(
    ...
    if not name_clean:
        ...
        return RedirectResponse(f"/?kyc_err={err}#profile", status_code=303)
    ...
    return RedirectResponse(f"/?kyc_msg={msg}#profile", status_code=303)
