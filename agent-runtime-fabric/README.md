...
# OR standard desktop: uvicorn app:app --reload --port 8080
```

Open `frontend/index.html` and set API base to `http://localhost:8080`.


### Mobile/iOS shell compatibility

If `uvicorn app:app` fails with signal-handler errors, use:

```bash
ARF_DISABLE_SIGNALS=1 python run_server.py
```
