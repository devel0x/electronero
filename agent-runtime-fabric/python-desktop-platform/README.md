...
2. **react-vite-fastapi** — full-stack modern web desktop template with React/Vite frontend + FastAPI backend.
3. **dashboard-app** — cards/charts, auth shell, table/grid basics.
4. **ai-chat-client** — streaming chat UI, model adapter interface, local history.
5. **local-first-crud** — SQLite repo, sync hooks, optimistic UI.
6. **settings-utility** — tray + settings pages + background checks.
7. **ide-workbench** — split panes, file tree, terminal process adapter.
8. **admin-ops-tool** — role-aware views, audit trail panel, task runners.
