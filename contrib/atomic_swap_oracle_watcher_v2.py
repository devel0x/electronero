...
``OP_RETURN`` output. When detected, the amount received is transferred
to that Monero address using the wallet RPC. If ``PRICE_API_URL`` is
set, the amount is first multiplied by a rate fetched from that API.
"""
...
def fetch_exchange_rate(url: str) -> float:
    ...
    The API should return JSON like ``{"rate": 1.23}``. If the request
    fails, ``1.0`` is returned.
    """
    if not url:
        return 1.0
    try:
...
class OracleWatcher:
    ...
    def poll(self) -> None:
        ...
        for tx in txs:
            ...
            atomic_amount = int(amount * rate * 1_000_000_000_000)
            params = {"destinations": [{"address": chainb_address, "amount": atomic_amount}]}
            self.chainb_rpc.call("transfer", params)
            print(f"Sent {amount * rate} XMR to {chainb_address} for tx {txid}")
            self.seen.add(txid)
...
def main() -> None:
    ...
    watcher = OracleWatcher(
        cfg["oracle"], chaina, chainb, cfg["interval"], cfg["price_api_url"]
    )
    watcher.run()
