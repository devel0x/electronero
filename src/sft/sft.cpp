...
bool sft_store::save(const std::string &file) {
    ...
    try
    {
        ...
        sft_store_data data{tokens, transfer_history};
        oa << data;
    }
    catch(const std::exception &e)
    {
...
sft_info *sft_store::get(const std::string &name) {
    auto it = tokens.find(name);
    if (it == tokens.end()) return nullptr;
    return &it->second;
}

const sft_info *sft_store::get(const std::string &name) const {
    auto it = tokens.find(name);
    if (it == tokens.end()) return nullptr;
    return &it->second;
}
...
bool sft_store::transfer(const std::string &name, const std::string &from, const std::string &to, uint64_t amount) {
    sft_info *tok = get(name);
    if (!tok) return false;
    auto fit = tok->balances.find(from);
    if (fit == tok->balances.end() || fit->second < amount) return false;
    fit->second -= amount;
    tok->balances[to] += amount;
    record_transfer(tok->address, from, to, 0, amount);
    cryptonote::address_parse_info info;
    cryptonote::network_type net;
...
bool sft_store::transfer_by_address(const std::string &address, const std::string &from, const std::string &to, uint64_t amount) {
    sft_info *tok = get_by_address(address);
    if (!tok) return false;
    auto fit = tok->balances.find(from);
    if (fit == tok->balances.end() || fit->second < amount) return false;
    fit->second -= amount;
    tok->balances[to] += amount;
    record_transfer(address, from, to, 0, amount);
    cryptonote::address_parse_info info;
    cryptonote::network_type net;
    if (parse_any_address(to, info, net) && info.has_payment_id)
    {
...
bool sft_store::approve(const std::string &name, const std::string &owner, const std::string &spender, uint64_t amount, const std::string &caller) {
    sft_info *tok = get(name);
    if (!tok) return false;
    if (caller != owner)
        return false;
...
bool sft_store::transfer_from(const std::string &name, const std::string &spender, const std::string &from, const std::string &to, uint64_t amount) {
    sft_info *tok = get(name);
    if (!tok) return false;
...
bool sft_store::transfer_from_by_address(const std::string &address, const std::string &spender, const std::string &from, const std::string &to, uint64_t amount) {
    sft_info *tok = get_by_address(address);
    if (!tok) return false;
    auto &allowed = tok->allowances[from][spender];
    if (allowed < amount) return false;
...
bool sft_store::approve_id(const std::string &address, uint64_t id, const std::string &owner, const std::string &spender, uint64_t amount, const std::string &caller)
{
    sft_info *tok = get_by_address(address);
    if(!tok) return false;
    if(caller != owner) return false;
    sft_unit &unit = tok->ids[id];
    auto fit = unit.balances.find(owner);
    if(fit == unit.balances.end()) return false;
    unit.allowances[owner][spender] = amount;
    return true;
}

bool sft_store::transfer_id_by_address(const std::string &address, uint64_t id, const std::string &from, const std::string &to, uint64_t amount)
{
    sft_info *tok = get_by_address(address);
    if(!tok) return false;
    sft_unit &unit = tok->ids[id];
    auto fit = unit.balances.find(from);
    if(fit == unit.balances.end() || fit->second < amount) return false;
    fit->second -= amount;
    unit.balances[to] += amount;
    record_transfer(address, from, to, id, amount);
    cryptonote::address_parse_info info; cryptonote::network_type net;
    if(parse_any_address(to, info, net) && info.has_payment_id)
    {
        std::string base = cryptonote::get_account_address_as_str(net, info.is_subaddress, info.address);
        unit.allowances[to][base] += amount;
    }
    return true;
}

bool sft_store::transfer_from_id_by_address(const std::string &address, uint64_t id, const std::string &spender, const std::string &from, const std::string &to, uint64_t amount)
{
    sft_info *tok = get_by_address(address);
    if(!tok) return false;
    sft_unit &unit = tok->ids[id];
    auto &allowed = unit.allowances[from][spender];
    if(allowed < amount) return false;
    auto fit = unit.balances.find(from);
    if(fit == unit.balances.end() || fit->second < amount) return false;
    allowed -= amount;
...
uint64_t sft_store::balance_of_id(const std::string &address, uint64_t id, const std::string &account) const
{
    auto itn = address_index.find(address);
    if(itn == address_index.end()) return 0;
    auto itt = tokens.find(itn->second);
    if(itt == tokens.end()) return 0;
    auto itid = itt->second.ids.find(id);
    if(itid == itt->second.ids.end()) return 0;
    auto fit = itid->second.balances.find(account);
    return fit == itid->second.balances.end() ? 0 : fit->second;
}

bool sft_store::set_uri(const std::string &address, const std::string &creator, const std::string &uri)
{
    sft_info *tok = get_by_address(address);
...
std::string sft_store::uri(const std::string &address, uint64_t id) const
{
    auto itn = address_index.find(address);
    if(itn == address_index.end()) return std::string();
    auto itt = tokens.find(itn->second);
    if(itt == tokens.end()) return std::string();
    return itt->second.base_uri + std::to_string(id) + ".json";
}

uint64_t sft_store::balance_of(const std::string &name, const std::string &account) const {
    auto it = tokens.find(name);
...
uint64_t sft_store::balance_of_by_address(const std::string &address, const std::string &account) const {
    auto itn = address_index.find(address);
    if (itn == address_index.end()) return 0;
    return balance_of(itn->second, account);
}

uint64_t sft_store::allowance_of(const std::string &name, const std::string &owner, const std::string &spender) const {
    auto it = tokens.find(name);
    if (it == tokens.end()) return 0;
...
bool sft_store::burn(const std::string &address, const std::string &owner, uint64_t amount) {
    sft_info *tok = get_by_address(address);
    if(!tok) return false;
    ...
    record_transfer(address, owner, "", 0, amount);
    return true;
}

bool sft_store::mint(const std::string &address, const std::string &creator, uint64_t amount) {
    sft_info *tok = get_by_address(address);
    if(!tok || tok->creator != creator) return false;
    tok->total_supply += amount;
    tok->balances[creator] += amount;
    record_transfer(address, "", creator, 0, amount);
    return true;
}

bool sft_store::mint_id(const std::string &address, const std::string &creator, uint64_t id, uint64_t amount) {
    sft_info *tok = get_by_address(address);
    if(!tok || tok->creator != creator) return false;
    sft_unit &unit = tok->ids[id];
    unit.total_supply += amount;
    unit.balances[creator] += amount;
    record_transfer(address, "", creator, id, amount);
    return true;
}

bool sft_store::set_creator_fee(const std::string &address, const std::string &creator, uint64_t fee)
{
    sft_info *tok = get_by_address(address);
...
bool sft_store::transfer_ownership(const std::string &address, const std::string &creator, const std::string &new_owner)
{
    sft_info *tok = get_by_address(address);
    if(!tok || tok->creator != creator)
        return false;
...
void sft_store::list_by_creator(const std::string &creator, std::vector<sft_info> &out) const {
...
{
    std::string base = make_sft_extra(op, fields);
    crypto::hash h;
                       ...
                       crypto::signature &sig, bool &has_sig)
{
    ...
    if(!(iss >> op_int))
        return false;
    op = static_cast<sft_op_type>(op_int);
    if (iss.peek() == '\t')
        iss.get();
    std::string field;
...
void sft_store::record_transfer(const std::string &token_address, const std::string &from, const std::string &to, uint64_t id, uint64_t amount)
{
    sft_transfer_record rec;
...
void sft_store::history_by_token(const std::string &token_address, std::vector<sft_transfer_record> &out) const
{
...
void sft_store::history_by_account(const std::string &account, std::vector<sft_transfer_record> &out) const
{
    for(const auto &r : transfer_history)
...
void sft_store::history_by_token_account(const std::string &token_address, const std::string &account, std::vector<sft_transfer_record> &out) const
{
    for(const auto &r : transfer_history)
