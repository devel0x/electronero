...
void GenerateBitcoins(bool fGenerate, CConnman* connman, int nThreads, const std::string& payoutAddress, CTxMemPool& mempool)
{
    ...
    std::thread([=, &mempool]() {
        while (fGenerating && !ShutdownRequested()) {
            ...
            LogPrintf("Launching %d miner threads 🚀...\n", nThreads);
            ...
            for (int threadId = 0; threadId < nThreads; ++threadId) {
                std::thread([=, &mempool]() {
                    ...
                    unsigned int lastTxUpdate = mempool.GetTransactionsUpdated();
                    ...
                    while (!ShutdownRequested() && fGenerating && !foundBlock.load()) {
                        ...
                        if (now - lastTemplateTime >= templateRefreshInterval ||
                            mempool.GetTransactionsUpdated() != lastTxUpdate ||
                            ::ChainActive().Height() != lastHeight) {
                            lastTemplateTime = now;
                            lastTxUpdate = mempool.GetTransactionsUpdated();
                            ...
                            for (uint32_t nonce = threadId; nonce < std::numeric_limits<uint32_t>::max(); nonce += nThreads) {
                                ...
                                if (nonce % 1000 == 0) {
                                    ...
                                    if (mempool.GetTransactionsUpdated() != lastTxUpdate ||
                                        ::ChainActive().Height() != lastHeight) {
                                        break;
                                    }
                                    ...
                                    if (elapsed >= 5000 && hashesDone > 0) {
                                        ...
                                        LogPrintf("⚡ Hashrate [thread %d]: %.2f H/s\n", threadId, rate);
                                        hashesDone = 0;
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    LogPrintf("🛠️ CreateNewBlock(): block weight: %u txs: %u fees: %ld sigops %d\n", GetBlockWeight(*pblock), nBlockTx, nFees, nBlockSigOpsCost);
