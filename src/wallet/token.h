...
struct TokenLedgerState {
    ...
    {
        READWRITE(obj.balances, obj.allowances, obj.totalSupply, obj.token_meta, obj.history, obj.governance_wallet, obj.governance_fees, obj.fee_per_vbyte, obj.create_fee_per_vbyte, obj.wallet_signers);
    }
};
...
private:
    ...
    void RegisterToken(const std::string& token, const std::string& name, const std::string& symbol, uint8_t decimals, const std::string& owner);
