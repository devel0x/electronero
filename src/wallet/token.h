...
struct TokenOperation {
    ...
    {
        READWRITE(obj.op, obj.from, obj.to, obj.spender, obj.token, obj.amount, obj.name, obj.symbol, obj.decimals);
    }
};
...
struct TokenLedgerState {
    ...
    std::string governance_wallet;
    CAmount governance_fees{0};
    CAmount fee_per_vbyte{1};

    SERIALIZE_METHODS(TokenLedgerState, obj)
    {
        READWRITE(obj.balances, obj.allowances, obj.totalSupply, obj.token_meta, obj.history, obj.governance_wallet, obj.governance_fees, obj.fee_per_vbyte);
    }
};
...
private:
    void CreateToken(const std::string& wallet, const std::string& token, CAmount amount, const std::string& name, const std::string& symbol, uint8_t decimals);
    void RegisterToken(const std::string& token, const std::string& name, const std::string& symbol, uint8_t decimals);
