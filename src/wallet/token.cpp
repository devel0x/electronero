...
bool TokenLedger::VerifySignature(const TokenOperation& op) const
{
    ...
    // ensure the signer matches the registered address for this wallet
    const std::string& wallet = op.op == TokenOp::TRANSFERFROM ? op.spender : op.from;
    auto it = m_wallet_signers.find(wallet);
    if (it == m_wallet_signers.end() || it->second != op.signer) {
        LogPrintf("❌ VerifySignature: Signer %s unauthorized for wallet %s\n", op.signer, wallet);
        return false;
    }
