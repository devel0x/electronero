...
void TokenLedger::RegisterToken(const std::string& token, const std::string& name, const std::string& symbol, uint8_t decimals, const std::string& owner)
{
    m_token_meta[token] = {name, symbol, decimals, owner};
}

void TokenLedger::CreateToken(const std::string& wallet, const std::string& token, CAmount amount, const std::string& name, const std::string& symbol, uint8_t decimals)
{
    m_balances[{wallet, token}] += amount;
    m_totalSupply[token] += amount;
    if (m_token_meta.count(token) == 0) {
        RegisterToken(token, name, symbol, decimals, wallet);
    }
}
...
bool TokenLedger::ApplyOperation(const TokenOperation& op, bool broadcast)
{
    ...
    switch (op.op) {
    case TokenOp::CREATE:
        CreateToken(op.from, op.token, op.amount, op.name, op.symbol, op.decimals);
        break;
    ...
    case TokenOp::MINT: {
        ...
        CreateToken(op.from, op.token, op.amount, it->second.name, it->second.symbol, it->second.decimals);
        break;
    }
    }
    if (ok) {
...
bool TokenLedger::Load()
{
    ...
    m_governance_wallet = state.governance_wallet;
...
bool TokenLedger::Flush() const
{
    ...
    state.governance_wallet = m_governance_wallet;
