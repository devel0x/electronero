...
from .models import HealthCheckResult, NodeRead, RewardRecord, UptimeScore
...
class HealthMonitor:
    ...
    async def _probe_all_nodes(self) -> None:
        ...
        if not nodes:
            return
        await asyncio.gather(*(self._probe_node(node) for node in nodes))
...
class RewardEngine:
    ...
    async def distribute_rewards(self) -> List[RewardRecord]:
        ...
        for node in nodes:
            ...
            scores[node.id] = uptime
            total_score += score
        if total_score == 0:
            return []
        pool = self.settings.reward_pool_daily
        records: List[RewardRecord] = []
        for node in nodes:
            uptime = scores.get(node.id)
            if not uptime:
                continue
            score = uptime.uptime_ratio * (1.0 if uptime.average_latency_ms is None else max(0.1, 1.5 - (uptime.average_latency_ms / 1500)))
            ...
            await self.repo.record_reward(record.dict())
