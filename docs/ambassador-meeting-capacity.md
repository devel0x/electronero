...
Administrators who log in through the `/admin` portal receive a **Meeting Control Center** card. From there you can:

- View every active room, including the email, display name, and join time for each participant.
- Remove a specific ambassador with a single click (optionally include a short reason so they see why they were removed).
- Shut down an entire room, which notifies all connected ambassadors and cleanly closes their WebSocket sessions.

The same data is also exposed programmatically at `GET /api/admin/meetings` for custom dashboards or alerting. Requests must include a valid admin session cookie.
