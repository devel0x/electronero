...
Electronero now includes a simple XRC-20 token platform accessible from the CLI and RPC. Use `token_create`, `token_balance`, `token_transfer`, `token_approve`, `token_transfer_from`, `token_set_fee`, `token_info`, `token_history`, `token_history_addr`, `all_tokens`, `my_tokens`, plus governance commands `proposal_create`, `proposal_vote`, `proposal_end`, `proposal_info`, `proposals`, and `active_proposals` to manage ERC20-like tokens and on-chain votes.
...
Tokens can be burned or minted. The wallet now supports simple on-chain proposals and voting. Use `proposal_create` to open a proposal (costing `PROPOSAL_CREATE_FEE`), `proposal_vote` to cast a vote (costing `PROPOSAL_VOTE_FEE`), `proposal_end` to close your proposal, `proposal_info` to view a proposal by id or title, `proposals` to list all proposals and `active_proposals` to show those still open.

`proposal_create <title>` expects a file named `<title>.json` in your wallet directory containing:

```json
{
  "title": "Proposal title",
  "description": "This is a description, it could be a few sentences or more but let's limit the byte size of the payload so users don't spam the blockchain with big data",
  "signature": "optional author signature"
}
```

The JSON payload is included on-chain and must be under 1&nbsp;MB. `proposal_info` displays the creator, description, signature, the current yes and no vote totals, and whether the proposal is active or ended.
