...
Tokens may offer staking rewards. The token creator sets a `reward_rate` with
`token_set_reward`. Token holders lock an amount using `token_stake` and later
call `token_unstake` to withdraw the original stake plus rewards. Rewards are
minted to the total supply when withdrawn. Staking is disabled whenever the
reward rate is zero.

Example usage:

```
token_set_reward <token_address> 5
token_stake <token_address> 100
# ... wait for rewards to accrue ...
token_unstake <token_address>
```
