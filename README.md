...
* `message_send <to_address> <file.json>` – send a message. The JSON file must contain `Subject` and `message` fields and must be under 1 MB. Sending a message pays `MESSAGE_SEND_FEE` (default `1000000`) to `GOVERNANCE_WALLET_ADDRESS` plus the normal network fee.
* `message_list` – list messages received by this wallet.
* `message_read <id|subject>` – display a single message.
