...
Tokens are created with a `name`, `symbol`, and initial `supply`. After paying the fee the wallet displays the token's `cEVM` address derived from the creator's wallet. Token data is stored in `~/.bitelectronero/tokens.bin` and synchronized across peers. Use `all_tokens` to view every token known to the wallet and `my_tokens` to see those you created.
