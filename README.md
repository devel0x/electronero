...
Every contract maintains its own balance tracked by the EVM. You can deposit coins with `call_contract <address> deposit:<amount> write`. Transfer between contracts uses `call_contract <address> transfer:<dest>:<amount> write` where `<dest>` is another contract address. These text commands bypass hex encoding and simply modify balances.
