...
Electronero now includes a simple token platform accessible from the CLI and RPC. Use `token_create`, `token_balance`, `token_transfer`, `token_approve`, `token_transfer_from`, and `token_info` commands to manage ERC20-like tokens.
...
Tokens are created with a `name`, `symbol`, and initial `supply`. Token data is stored in `~/.bitelectronero/tokens.bin` and synchronized across peers.
