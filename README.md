...
Amounts for `deposit_contract` and the `deposit:` and `transfer:` text commands use the wallet's current display unit (for example `1` equals `1.00000000`). Both the CLI and wallet RPC server convert these values to atomic units before sending them to the daemon.
This normalization ensures contract balances are updated precisely and prevents leftover fractions after a transfer.
