...
Electronero now includes a simple token platform accessible from the CLI and RPC. Use `token_create`, `token_balance`, `token_transfer`, `token_approve`, `token_transfer_from`, `token_info`, `token_history`, `token_history_addr`, `all_tokens`, and `my_tokens` commands to manage ERC20-like tokens.
...
Every `token_transfer` and `token_transfer_from` also pays a small governance fee defined by `TOKEN_TRANSFER_FEE` to the same address.
