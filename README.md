...
Electronero now includes a simple XRC-20 token platform accessible from the CLI and RPC. Use `token_create`, `token_balance`, `token_transfer`, `token_approve`, `token_transfer_from`, `token_set_fee`, `token_pause`, `token_unpause`, `token_info`, `token_history`, `token_history_addr`, `all_tokens`, `tokens_deployed`, and `my_tokens` commands to manage ERC20-like tokens.
...
* `token_unpause <token_address>` – resume transfers for a token, also paying `TOKEN_DEPLOYMENT_FEE`.
