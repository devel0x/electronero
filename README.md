...
Electronero now includes a simple XRC-20 token platform accessible from the CLI and RPC. Use `token_create`, `token_balance`, `token_transfer`, `token_approve`, `token_transfer_from`, `token_set_fee`, `token_lock_creator_fee`, `token_info`, `token_history`, `token_history_addr`, `all_tokens`, and `my_tokens` commands to manage ERC20-like tokens.
...
`token_lock_creator_fee` permanently locks the current creator fee after paying the deployment fee so it can no longer be changed.
...
Use `token_info` with the token address to query a token's metadata, including whether the creator fee is locked.
...
* `token_lock_creator_fee <token_address>` – lock the creator fee forever; pays `TOKEN_DEPLOYMENT_FEE`.
