...
While some snippets below rely on inline `assembly` to demonstrate custom opcodes, contracts do **not** need assembly to run on the EVM. Standard Solidity code compiles and executes just fine. Recent updates expanded opcode coverage to include jumps and bitwise instructions so most contracts written for Solidity 0.8 work out of the box.
